...
1. Create a free-forever Cassandra Database with DataStax Astra. Use the keyspace name `ks1` and the username/password combo `datastax/datastax` when creating the db to make things easy.  link:https://astra.datastax.com/register?utm_source=devplay&utm_medium=github&utm_campaign=java-framework-compare[click here to get started] 🚀
